...
void Player::initialHandShake() {
  *toAI << side << endl;
  ...
  if (autoPlay && system(pause.c_str()) < 0) {
...
void Player::receiveActionCommands() {
  ...
  int weapon;
  if (alive) {
    if (autoPlay && system(resume.c_str()) < 0) {
      throw
    ...
    chrono::system_clock::time_point till =
      chrono::system_clock::now() +
      chrono::milliseconds(responseTimeLimit);
    while (!done) {
      this_thread::sleep_for(chrono::milliseconds(1));
      chrono::system_clock::time_point now = 
	chrono::system_clock::now();
  ...
  FINISH:
    ...
    gameLog.addTurnPlay(weapon, orderSave, 0, territories);
    return;
  } else {
    *dump << "!!! Disqualified";
  }
